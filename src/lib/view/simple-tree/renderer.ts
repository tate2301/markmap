...
export class SimpleTreeRenderer {
  ...
  private updatePartialRender(nodeKeys: string[]) {
    ...
    const tree = flextree({
      ...
      siblingSpacing: Math.max(options.siblingSpacing ?? NODE_PADDING, NODE_PADDING),
      nodeSize: (node: FlexTreeNode) => {
        ...
        const size: [number, number] = storedSize 
          ? [storedSize.width + NODE_PADDING, storedSize.height + NODE_PADDING]
  ...
  private updateViewport() {
    ...
    this.currentViewport = {
      x: -transform.x / transform.k - VIEWPORT_OVERSCAN,
      y: -transform.y / transform.k - VIEWPORT_OVERSCAN,
      width: (svgRect.width / transform.k) + (VIEWPORT_OVERSCAN * 2),
      height: (svgRect.height / transform.k) + (VIEWPORT_OVERSCAN * 2)
    };
  ...
  centerNode(node: IEnhancedNode): void {
  ...
  ensureVisible(node: IEnhancedNode): void {
    const nodeElement = this.findElement(node);
    if (!nodeElement) return;
  ...
  findElement(node: IEnhancedNode): SVGGElement | null {
